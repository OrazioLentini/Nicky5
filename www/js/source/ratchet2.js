  ...
  if (window.navigator.msPointerEnabled) {
	window.addEventListener('MSPointerUp', function (event) {
		...
		if (modal) {
		  ...
		  if (modal && modal.classList.contains('modal-left')) {
			modal.classList.toggle('active-left');
		  }
		  if (modal && modal.classList.contains('modal-menu')) {
...
if (window.navigator.msPointerEnabled) {
	window.addEventListener("MSPointerUp", showHidePopover);
}
else {
...
!(function () {
  ...
  // ==========================
if (window.navigator.msPointerEnabled) {
	window.addEventListener('MSPointerDown', function () { isScrolling = false; });
  window.addEventListener('MSPointerMove', function () { isScrolling = true; });
...
var onTouchStartWindows = function (e) {
    slider = getSlider(e.target);
 ...
 var onTouchMoveWindows = function (e) {
	 console.log(e)
   // if (e.touches.length > 1 || !slider) {
     // return; // Exit if a pinch || no slider
   // }
...
if (window.navigator.msPointerEnabled) {
  window.addEventListener("MSPointerDown", onTouchStartWindows);
  window.addEventListener("MSPointerMove", onTouchMoveWindows);
  window.addEventListener("MSPointerUp", onTouchEnd);
}
else {
...
/**
    * Non-webkit vendor support for Push.js
    * --------------------------------------
    * Check if webkitTransition is supported,
    * If not, add transitionend listeners to 
    * every new .content element added, and
    * dispatch the webkitTransitionEnd listeners.
    */
(function(D,P){
    if(typeof(D.body||D.documentElement).style[P]!=='string') {
        window.onload = function(){
            ...
            observer = new MutationObserver(function(mutations) {
              mutations.forEach(function(mutation) {
                  ...
                  if(nodes[i].classList && nodes[i].classList.contains('content'))
                nodes[i].addEventListener('transitionend', function(e){
                    e.target.dispatchEvent(new CustomEvent('webkitTransitionEnd', {
                            bubbles: true,
                            cancelable: true
                        }));
                });
